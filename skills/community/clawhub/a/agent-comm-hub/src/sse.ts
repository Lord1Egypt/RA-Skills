...
import { pushToWebSocket } from "./websocket.js";
 ...
 *
 * 优先使用 SSE 推送；如果 SSE 连接不存在或失败，尝试 WebSocket。
 *
...
export function pushToAgent(agentId: string, event: object, dedupId?: string): boolean {
  ...
  if (res) {
    try {
      ...
      // 不返回 false — 继续尝试 WebSocket
    }
  }

  // SSE 不可用，尝试 WebSocket
  return pushToWebSocket(agentId, event);
}
