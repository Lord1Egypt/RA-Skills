 ...
 */
function runRules(diffFiles, logEntries, task, strict, silent, commitMsg, config) {
    const ctx = { diffFiles, logEntries, task, strict, silent, commitMsg, config };
    const results = [];
    // 根据 config.extendedRulesEnabled 决定运行哪些规则
    const rulesToRun = config?.extendedRulesEnabled
        ? rules_1.rules
    ...
    for (const rule of results) {
        if (rule.status === 'FAIL')
            exitCode = 2;
        else if (rule.status === 'WARN' && exitCode === 0)
            exitCode = 1;
    }
