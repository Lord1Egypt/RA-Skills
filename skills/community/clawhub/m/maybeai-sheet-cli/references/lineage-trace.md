...
```text
POST /api/v3/excel/formula/lineage_trace
```

`formula/lineage_trace` is a read-only formula dependency endpoint.
...
The public endpoint to use is `/api/v3/excel/formula/lineage_trace`.
...
{
  "document_id": "<document_id>",
  "targets": [{"worksheet_name": "Report", "cell": "C2"}],
  "format": "tree"
}
```
