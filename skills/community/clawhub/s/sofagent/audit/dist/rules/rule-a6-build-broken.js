...
exports.checkRuleA6 = checkRuleA6;
...
function checkRuleA6(ctx) {
    ...
    for (const file of diffFiles) {
        if (!isBuildConfigFile(file.path))
            continue;
        // 统计删除行数（以 - 开头且不是 ---）
        let deletionCount = 0;
    ...
    if (flaggedFiles.length > 0) {
        rule.status = 'WARN';
        rule.details.push(`构建配置文件被破坏性修改: ${flaggedFiles.join(', ')}。删除行数超过 ${DELETION_THRESHOLD} 行，请确认不会导致构建失败。`);
    }
    return rule;
}
//# sourceMappingURL=rule-a6-build-broken.js.map