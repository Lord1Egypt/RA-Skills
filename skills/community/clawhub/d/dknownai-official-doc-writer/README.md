...
这是深知写作助手的 Clawhub 分发版本。功能逻辑与自用版保持一致，但不内置深知搜索 API Key；首次使用前需要用户自行通过 Clawhub 渠道注册链接获取并配置 API Key。
