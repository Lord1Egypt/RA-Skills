...
# ===== 配置 =====
PROXY_URL = "https://1439498936-23pvh3iikx.ap-guangzhou.tencentscf.com"
...
def tool_get_seckill_items(params):
    ...
    if not data.get("ok"):
        if data.get("error") == "timeout":
            ...
            if not data.get("ok"):
                return json.dumps({
                    "content": "",
                    "summary": "正在为您精选秒杀好货，数据量较大，请稍候约30秒后再次查询。"
                }, ensure_ascii=False)
        else:
            return json.dumps({
                "content": "",
    ...
    if not items and total == 0:
        ...
        if keyword:
            ...
            msg = f'未找到与「{keyword}」相关的秒杀好货。换个关键词试试'
            if cat_str:
                msg += f'，或浏览热门分类：{cat_str}'
            else:
                msg += '。'
        else:
            msg = "暂无符合条件的秒杀好货，请稍后再试。"
    ...
    if keyword and total <= SCARCITY_THRESHOLD:
        # 稀缺，全展示不分页
        if cond_str:
            hints.append(f'{cond_str}的秒杀好货仅{total}条，{quality_desc}，手慢无！')
        else:
            hints.append(f'秒杀好货仅{total}条，{quality_desc}，手慢无！')
        cat_str = _build_category_overview(category_overview)
        if cat_str:
        ...
        # 带品类词，结果较多
        if cond_str:
            hints.append(f'{cond_str}的秒杀好货共{total}条，{quality_desc}。')
        else:
            hints.append(f'秒杀好货共{total}条，{quality_desc}。')
        if has_more:
