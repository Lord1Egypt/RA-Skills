...
| `logout` | 仅卸载时由 `uninstall.sh` 内部调用 | 通过 access_token 直接、静默地撤销本机授权，随后清除本地登录态 | 是 |
...
当用户要求**卸载 CLI / 清除本地环境 / 清空所有数据并重置**时，通过 `uninstall.sh` 完成。它会依次：

1. 调用 `logout` 在服务端撤销本设备授权。
2. 删除本地安装目录（含登录态 / access_token / 账号信息）。
3. 移除全局命令 `quarkclouddrive` 与 shell 配置中的 PATH 条目。

```bash
bash uninstall.sh
```

> **二次确认（必须执行，不可跳过）**：卸载会撤销授权并清除本地信息，属于**不可逆**操作。agent 在执行 `uninstall.sh` 之前，必须先用自然语言明确告知用户即将发生什么、会清除哪些信息，并取得用户的二次确认后才能调用。
>
> 告知话术按以下文案输出（用自然语言表达，不要暴露文件路径等技术细节）：
>
> 确认卸载夸克网盘 Skill？
> 卸载将执行以下操作：解除网盘账号对当前设备的授权 • 清除本地配置与缓存。
> 你的网盘文件不会受到影响。如需再次使用，重新安装 Skill 并完成授权即可。
>
> 仅当用户明确回复「确认 / 是 / 继续卸载」等肯定意图后，才执行 `bash uninstall.sh`。若用户只是想**取消授权但保留 CLI**，应改用 `unauthorize`，不要卸载。
