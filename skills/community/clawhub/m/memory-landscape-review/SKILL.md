---
name: "Memory Landscape Review"
description: "Use when the user wants to review auto-memory, promote durable instructions into CLAUDE.md or local memory, and clean up duplicates or conflicts."
category: "other"
source: "ClawHub"
tags: [claude-code, extracted]
platforms: []
author: ""
version: ""
license: ""
installCmd: "hermes skills install clawhub/memory-landscape-review"
sourceUrl: "https://clawhub.ai/skills/memory-landscape-review"
---

# Memory Landscape Review

> Use when the user wants to review auto-memory, promote durable instructions into CLAUDE.md or local memory, and clean up duplicates or conflicts.

- **Category:** Other
- **Source:** ClawHub
- **Author:** 
- **Version:** 
- **License:** 
- **Platforms:** All
- **Install Command:** `hermes skills install clawhub/memory-landscape-review`
- **Source URL:** [https://clawhub.ai/skills/memory-landscape-review](https://clawhub.ai/skills/memory-landscape-review)

## Overview


## Installation
To install this skill, run the following command in your terminal:
```bash
hermes skills install clawhub/memory-landscape-review
```
