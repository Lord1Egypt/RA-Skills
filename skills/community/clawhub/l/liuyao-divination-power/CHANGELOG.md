...
### 算法修正 — 旺相休囚死五分制
- **月建五分制打分**：旺（当令）+2、相（得生）+1、休（泄气）0、囚（被制）-1、死（被克）-1（`jie-gua-xiang-jie.md` 第二节）
- **日破降权**：日破从 -2 降为 -1.5，轻于月破 -2，区分日月破损严重程度
- **综合判定阈值调整**：旺相有力 ≥ +2、中和可用 -1~+1、休囚无力 ≤ -1.5、空破无用（特殊条件）
- 新增四档判定速查表，含 12 种常见月日组合场景

### Skill 架构改进 — 强制工具加载
- **铁律0**：工具加载优先于预训练记忆——每步进入分析前必须使用 `read_file` 工具加载对应 reference 文件（`SKILL.md`）
- 铁律5、7 显式化 `read_file` 工具调用措辞
- 铁律10 示例分值更新为五分制（旺+2/相+1/休0/囚-1/死-1/日破-1.5）
- 全部 9 个步骤的「⛔ 强制加载」改为「⛔ 强制加载（工具调用）」+ 显式 `read_file("...")` 指令
- 统一第3.5~七步中全部 6 处「额外加载」为「额外执行 `read_file(...)`」（消除与主加载指令的格式不一致）

### 校验增强
- 第八步新增 **D 项「规则来源追溯」**：逐步检查每步是否正确使用了 reference 中的规则（五分制/合保护/十二长生/刑害/高级应期/场景类象）

### Reference 强化
- `jie-gua-xiang-jie.md` 第一节、第二节各加入 ⚠️ 差异声明锚点（标注与通行六爻论著的差异）
- `dong-bian-fa-ze.md` 月破描述从「与日破对等」改为「重于日破」
