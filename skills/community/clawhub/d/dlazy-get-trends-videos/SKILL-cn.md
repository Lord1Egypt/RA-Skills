...
version: 1.2.0
description: 调用 Apify 抓取 YouTube 或 TikTok 的热门视频列表，按关键词、时间窗口与排序返回 N 条视频的元数据（标题、作者、播放量、缩略图、页面链接等）。仅返回元数据，不下载文件；如需获取可用的视频文件，请把选中的 pageUrl 传给 download-trends-videos。
metadata:
  {
    'clawdbot':
      {
    ...
    'openclaw': { 'systemPrompt': '当调用此技能时，可以使用 dlazy get-trends-videos -h 查看帮助信息。' },
  }
---
...
调用 Apify 抓取 YouTube 或 TikTok 的热门视频列表，按关键词、时间窗口与排序返回 N 条视频的元数据（标题、作者、播放量、缩略图、页面链接等）。仅返回元数据，不下载文件；如需获取可用的视频文件，请把选中的 pageUrl 传给 download-trends-videos。
...
执行 `dlazy get-trends-videos` 命令获取结果。

```bash
dlazy get-trends-videos -h

Options:
  --source [source]                    数据源：youtube 或 tiktok [default: tiktok] (choices: "youtube", "tiktok")
  --query [query]                      搜索关键字 [default: ]
  --maxResults [maxResults]            最多返回的视频数量（1-50） [default: 10]
  --dateFilter [dateFilter]            时间窗口：today / week / month / year / all [default: week] (choices: "today", "week", "month", "year", "all")
  --sort [sort]                        排序方式：relevance（相关性）/ newest（最新）/ popular（最热） [default: popular] (choices: "relevance", "newest", "popular")
  --downloadVideos [downloadVideos]    TikTok 专用：是否让 Apify Actor 顺带把视频文件下载到 KVS，便于后续直接拉取视频地址；YouTube 忽略此项 [default: true]
...
```bash
dlazy get-trends-videos --prompt '提示词内容'
```
