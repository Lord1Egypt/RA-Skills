# -*- coding: utf-8 -*-
"""
实时热销代理 v5.0 - 腾讯云SCF Web函数
函数名：jd-trending-proxy
v5.0 变更：SCF端做筛选+排序+分页，只返回一页结果（解决6MB响应限制）

缓存逻辑：
- 首次请求：全量拉取京东API(~30秒)，硬过滤后缓存
...
- 好评率筛选（默认≥98%，固定不降级）
- 销量筛选（默认≥2000，可降级2000→1000→500→200→不限）
- 价格筛选（max_price）
...
# ===== 爆品标签集合 =====
HOT_TAGS = {"热销", "热卖", "爆款", "畅销", "人气", "超万人正在疯抢", "同款高转化", "热销榜", "热卖榜", "人气好物"}
...
def _calc_score_scf(g):
    """综合评分（满分100）
    销量30 + 折扣力度20 + 推广热度20 + 好评15 + 自营10 + 爆品标签5

    热销场景：销量权重最高，其次推广热度和折扣
    """
    ...
    # 折扣力度 0-20
    discount_score = min(discount * 0.4, 20)

    # 推广热度 0-20
    promo_score = min(comm_share * 1.5, 20)
    ...
    return round(sales_score + discount_score + promo_score + comments_score + jd_score + tag_score, 1)
