...
"""
速度优化脚本
用于优化 Tesseract OCR 模型和 OpenCV 图表解析模型的速度
"""
...
class SpeedOptimizer:
    def __init__(self, data_dir: str = "data"):
        ...
        self.speed_dir = self.data_dir / "speed"

        # 创建目录
        self.speed_dir.mkdir(parents=True, exist_ok=True)

    def optimize_tesseract_speed(self, model_name: str, config_path: str) -> Dict[str, Any]:
        """
        优化 Tesseract 速度
        """
        print(f"📊 优化 Tesseract 速度: {model_name}")
        ...
        # 保存优化参数
        optimize_path = self.speed_dir / f"{model_name}_speed.json"
        with open(optimize_path, "w", encoding="utf-8") as f:
            json.dump(optimization_params, f, ensure_ascii=False, indent=2)

        print(f"✅ Tesseract 速度优化参数已保存: {optimize_path}")
        return optimization_params

    def optimize_opencv_speed(self, model_name: str, model_path: str) -> Dict[str, Any]:
        """
        优化 OpenCV 速度
        """
        print(f"📊 优化 OpenCV 速度: {model_name}")
        ...
        print(f"✅ OpenCV 速度优化参数已保存: {optimize_path}")
        return optimization_params

    def optimize_all_speed(self) -> Dict[str, Any]:
        """
        优化所有速度
        """
        print("🚀 开始优化速度...")
        ...
        for model_name, config_path in tesseract_models.items():
            optimize_params = self.optimize_tesseract_speed(model_name, config_path)
        ...
        for model_name, model_path in opencv_models.items():
            optimize_params = self.optimize_opencv_speed(model_name, model_path)
        ...
        # 保存汇总
        with open(self.speed_dir / "all_speed.json", "w", encoding="utf-8") as f:
            json.dump(all_optimized, f, ensure_ascii=False, indent=2)

        print(f"✅ 所有速度优化参数已保存到 {self.speed_dir / 'all_speed.json'}")
...
if __name__ == "__main__":
    ...
    optimize_info = optimizer.optimize_all_speed()
    print("\n🎉 速度优化完成！")
