# -*- coding: utf-8 -*-
"""
京东超级补贴代理 v5.0 - 腾讯云SCF Web函数
函数名：jd-super-deals-proxy
...
- 好评率筛选（默认≥98%，可自定义99/100）
- 销量筛选（默认≥5000，MCP端控制降级）
- 价格筛选（max_price）
- 排序（score/price/subsidy）
- 分页（page/page_size）
...
# ===== 爆品标签集合 =====
HOT_TAGS = {"超补爆品", "同类超低价", "破价品", "超万人正在疯抢", "超级补贴", "同款高转化"}
...
def _calc_score_scf(g):
    """综合评分（满分100）
    补贴力度35 + 推广热度20 + 销量15 + 好评15 + 自营10 + 爆品标签5
    """
    subsidy = _get_item_subsidy(g)
    ...
    subsidy_score = min(subsidy * 2, 35)
    ...
    tag_score = min(sum(2 for t in tags if t in HOT_TAGS), 10)

    return round(subsidy_score + comm_score + sales_score + comments_score + jd_score + tag_score, 1)


def _sort_scf(items, sort_by):
    """排序：score=综合(默认)，price=到手价升序，subsidy=补贴金额降序"""
    if sort_by == "price":
        return sorted(items, key=lambda g: _get_item_price(g))
    elif sort_by == "subsidy":
        return sorted(items, key=lambda g: _get_item_subsidy(g), reverse=True)
    else:
...
def _filter_and_degrade(cached_data, keyword, min_gc_user):
    ...
    降级路径（min_good_comments=98时）：
      默认:  好评≥98% + 销量≥5000
      降级1: 好评≥98% + 销量≥1000
      降级2: 好评≥97% + 销量≥1000
      底线:  好评≥97% + 不限销量

    用户指定min_good_comments=99/100时，只降销量，好评率不降级。
