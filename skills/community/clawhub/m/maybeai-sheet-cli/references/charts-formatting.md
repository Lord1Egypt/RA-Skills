...
These endpoints are not CLI commands yet. Use `maybeai-sheet raw post <path> --body body.json`.
...
Use `maybeai-sheet raw post` for freeze panes, styles, and autofilter endpoints. Verify with `maybeai-sheet sheet read --output table`.
