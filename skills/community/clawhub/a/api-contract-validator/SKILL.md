...
description: "Free tier: API Contract Validator — Validate API contracts, check endpoints, response formats"
version: 1.0.0
metadata:
  openclaw:
    requires:
...
# 📋 API Contract Validator — Free Tier / 免费版
...
Validate API contracts, check endpoints, response formats
...
### Check Endpoints
\`\`\`bash
curl -sI https://api.example.com/health 2>/dev/null | head -5
# Check response format
curl -s https://api.example.com/status 2>/dev/null | python3 -m json.tool 2>/dev/null || echo "Not JSON"
\`\`\`

### Validate Headers
\`\`\`bash
curl -sI -X OPTIONS https://example.com/api 2>/dev/null | grep -i -E "^(access-control|content-type|rate)"
\`\`\`
