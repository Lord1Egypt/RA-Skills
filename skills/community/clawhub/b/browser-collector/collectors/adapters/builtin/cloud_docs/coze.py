#!/usr/bin/env python3
"""
collectors/adapters/builtin/cloud_docs/coze.py - Coze扣子文档适配器

支持: www.coze.com, www.coze.cn, coze.com, coze.cn 文档结构化提取
...
class CozeDocAdapter(DocAdapter):
    """
    Coze扣子文档适配器

    完整支持 coze.com/coze.cn 文档结构化提取。
    处理SPA页面，需等待内容加载完成。
    """
    ...
    KNOWN_DOC_SITES = {
        'www.coze.com': {
            'content_selector': '.markdown-body, .article-content, .docs-content',
            'heading_selector': 'h1, h2, h3, h4',
            'sidebar_nav': '.sidebar-nav, .catalog-nav, .docs-nav, #catalog',
    ...
    def before_navigate(self, page, url: str):
        """导航前准备 - SPA页面需要特殊处理"""
        super().before_navigate(page, url)

        # Coze文档是SPA，需要等待页面完全加载
        try:
            # 等待主要内容容器出现
            ...
            try:
                ...
                for item in nav_items:
                    ...
                    if text and len(text) > 1:
                        ...
                        # 跳过外部链接
                        if href.startswith('http') and 'coze' not in href:
                            continue
    ...
    def extract_breadcrumbs(self, page) -> List[str]:
        ...
        breadcrumb_selectors = [
            ...
            '.docs-breadcrumb a, .docs-breadcrumb span',
            '.document-path a, .document-path span',
...
# 注册适配器到全局注册表
_adapter_instance = CozeDocAdapter()
register_adapter('coze_doc', _adapter_instance)
register_adapter('coze', _adapter_instance)
register_adapter('www.coze.com', _adapter_instance)
register_adapter('www.coze.cn', _adapter_instance)


__all__ = ['CozeDocAdapter']
