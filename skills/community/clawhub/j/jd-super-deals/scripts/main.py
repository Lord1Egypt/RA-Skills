#!/usr/bin/env python3
"""京东超级补贴 - 补贴好货查询（缓存版 v3）

配合独立SCF函数 jd-super-deals-proxy 使用。
SCF负责：缓存全量数据(12h TTL) + 硬过滤(自营/旗舰店 + 好评≥97%)
         + 好评/销量动态筛选+降级 + 关键词匹配 + 排序 + 分页
MCP负责：发送参数、接收结果、构建用户提示语、转链

降级逻辑（SCF端执行，关键词结果<10条时触发，先降销量，好评97%是底线）：
  默认:  好评≥98% + 销量≥5000  (~1500条)
  降级1: 好评≥98% + 销量≥1000  (~3700条)
  降级2: 好评≥97% + 销量≥1000  (~4200条)
  底线:  好评≥97% + 不限销量   (~8800条)

用户指定min_good_comments=99/100时，好评率不降级，只降销量。
"""
...
# ===== 配置 =====
PROXY_URL = "https://1439498936-ebgg6n6ain.ap-guangzhou.tencentscf.com"
PROXY_TOKEN = "tp_8k2mX9vQ4z"
ELITE_ID = 12254  # 超级补贴频道
...
def tool_get_deals(params):
    ...
    if not data.get("ok"):
        if data.get("error") == "timeout":
            ...
            if not data.get("ok"):
                return json.dumps({
                    "content": "",
                    "summary": "正在为您精选超级补贴好货，数据量较大，请稍候约1分钟后再次查询。"
                }, ensure_ascii=False)
        else:
            return json.dumps({
                "content": "",
    ...
    if not items and total == 0:
        ...
        if keyword:
            ...
            msg = f'未找到与「{keyword}」相关的超级补贴好货。换个关键词试试'
            if cat_str:
                msg += f'，或浏览热门分类：{cat_str}'
            else:
                msg += '。'
        else:
            msg = "暂无符合条件的超级补贴好货，请稍后再试。"
    ...
    if keyword and total <= SCARCITY_THRESHOLD:
        # 场景3：稀缺，全展示不分页
        if cond_str:
            hints.append(f'{cond_str}的超级补贴好货仅{total}条，{quality_desc}，好货稀缺，先到先得！')
        else:
            hints.append(f'超级补贴好货仅{total}条，{quality_desc}，好货稀缺，先到先得！')
        cat_str = _build_category_overview(category_overview)
        if cat_str:
        ...
        # 场景2/4：带品类词，结果较多
        if cond_str:
            hints.append(f'{cond_str}的超级补贴好货共{total}条，{quality_desc}。')
        else:
            hints.append(f'超级补贴好货共{total}条，{quality_desc}。')
        if has_more:
