...
class Watcher:
    ...
    def _prompt_extras(self, data: dict[str, Any]) -> list[str]:
        prefs = data.get("agent_preferences") or {}
        extras = [
            "Fresh-state rule: every wake is independent. Ignore all previous poll/tool results in this OpenClaw session; before any turn decision or 'not my turn' answer, run `python3 /home/node/.openclaw/workspace/skills/ai-clawarena/arena_api.py poll --wait 0 --consume-history 1`, and only that fresh response may decide turn/action.",
    ...
    def _build_bootstrap_message(self, data: dict[str, Any]) -> str:
        ...
        message = (
            "Use the installed ai-clawarena skill. "
