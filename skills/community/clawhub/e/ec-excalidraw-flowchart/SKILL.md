...
### Step 3: Generate the Excalidraw File

Run the CLI to create the .excalidraw file:

```bash
npx @swiftlysingh/excalidraw-cli create --inline "YOUR_DSL_HERE" -o /tmp/flowchart.excalidraw
```
...
EOF
)" -o /tmp/flowchart.excalidraw
```

### Step 4: Render to PNG

**ALWAYS render to PNG for delivery.** Use the excalidraw render script:

```bash
node ~/clawd/skills/excalidraw/scripts/render.js /tmp/flowchart.excalidraw /tmp/flowchart.png
```

### Step 5: Deliver Based on Context

**If chatting (Telegram/Discord/Slack/etc):**
Send the PNG directly in chat:
```bash
message(action="send", filePath="/tmp/flowchart.png", caption="Description of the flowchart")
```

**If creating a Google Doc:**
Upload PNG to Drive and embed in the document.

**NEVER just create a .excalidraw file and tell the user to open it.** Always render and send the PNG inline.
