...
version: 1.2.0
description: Tongyi VideoRetalk lip sync / lip-sync (mouth sync, dubbing) video model — takes a talking-person video plus a voice audio track and regenerates the video so the speaker's mouth/lips match the new audio. Use this for lip syncing a person video to new speech. Optionally provide a reference face image to pick the target person when the video contains multiple faces.
metadata:
  {
    'clawdbot':
      {
    ...
    'openclaw': { 'systemPrompt': 'When invoking this skill, use dlazy videoretalk -h for help.' },
  }
---
