...
import { createWebSocketServer, drainWsClients } from "./websocket.js";
...
app.get("/health", (_req: Request, res: Response) => {
  ...
  let dbSize = 0;
  try {
    const row = getRow<{ size: number }>(db.prepare(`SELECT page_count * page_size as size FROM pragma_page_count(), pragma_page_size()`));
    dbSize = row?.size ?? 0;
  } catch {}
...
async function gracefulShutdown(signal: string): Promise<void> {
  ...
  // 2. drain SSE + WebSocket 连接
  drainAllClients();
  drainWsClients();
  logger.info("connections_drained", { module: "server" });
