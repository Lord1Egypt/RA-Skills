...
exports.checkRuleA5 = checkRuleA5;
...
function checkRuleA5(ctx) {
    ...
    if (ctx.commitMsg !== undefined) {
        ...
        try {
            message = (0, child_process_1.execFileSync)('git', ['log', '-1', '--pretty=%B'], { encoding: 'utf-8' }).trim();
        }
        catch (err) {
            rule.status = 'FAIL';
            rule.details.push('无法读取 commit message: ' + err.message);
            return rule;
        }
    }
    if (!message) {
...
//# sourceMappingURL=rule-a5-honest-report.js.map