...
import { getRow, getAllRows, getCount } from "./utils.js";
...
export function verifyToken(plainToken: string): AuthContext | null {
  ...
  const row = getRow<{ agent_id: string; role: string }>(
    db.prepare(
  ...
  // 检查是否过期
  const expiresRow = getRow<{ expires_at?: number }>(
    db.prepare(`SELECT expires_at FROM auth_tokens WHERE token_value=?`),
    hash
  );
  ...
  return { agentId: row.agent_id, role: row.role as AgentRole };
}
...
export function verifyInviteCode(plainCode: string): "admin" | "member" | null {
  ...
  const row = getRow<{ role: string; expires_at?: number }>(
    db.prepare(
  ...
  return row.role as "admin" | "member";
}
...
export function auditLog(action: string, agentId: string | null, target?: string, details?: string): void {
  ...
  try {
    ...
    const lastRow = getRow<{ record_hash: string }>(
      db.prepare(
...
export function verifyAuditChain(): {
  ...
  const rows = getAllRows<{ id: string; action: string; agent_id: string | null; target: string | null; details: string | null; prev_hash: string | null; record_hash: string | null; created_at: number }>(
    db.prepare(
...
export function recalculateTrustScore(agentId: string): number {
  const verifiedCaps = getCount(
    db.prepare(`SELECT COUNT(*) as cnt FROM agent_capabilities WHERE agent_id=? AND verified=1`),
    agentId
  );

  const autoStrategies = getCount(
  ...
  const rejectedApps = getCount(
    db.prepare(`SELECT COUNT(*) as cnt FROM audit_log WHERE action='apply_strategy' AND agent_id=? AND details LIKE '%fail%'`),
    agentId
  );

  const revokedTokens = getCount(
    db.prepare(`SELECT COUNT(*) as cnt FROM audit_log WHERE action='revoke_token' AND agent_id=?`),
    agentId
  );

  // 注意：strategy_feedback.agent_id 是反馈者，不是提案者
  // 要查"别人给该 agent 策略的反馈"需要 JOIN strategies.proposer_id
  const positiveFb = getCount(
    db.prepare(
