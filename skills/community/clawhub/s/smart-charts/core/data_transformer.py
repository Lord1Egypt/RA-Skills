...
3. 用户确认环节 — 执行前向用户展示代码摘要，需用户确认后才执行
4. 沙箱内置函数 — 仅暴露安全的内置函数，禁止 open/exec/eval/__import__ 等
"""
...
def format_code_preview(code: str, max_lines: int = 15) -> str:
    """格式化代码预览，用于用户确认。"""
    lines = code.strip().split('\n')
    if len(lines) <= max_lines:
        return code.strip()
    preview = '\n'.join(lines[:max_lines])
    preview += f"\n... (共 {len(lines)} 行，已省略 {len(lines) - max_lines} 行)"
    return preview
...
class DataTransformer:
    ...
    def __init__(self, auto_confirm: bool = False, timeout: int = None):
        """
        Args:
            auto_confirm: 是否跳过用户确认环节。
                          在 CLI 交互模式下应设为 False（默认），
                          在程序化调用（如批量生成）中可设为 True，
                          但黑名单和 AST 校验仍然生效。
    ...
    def transform(self, df: pd.DataFrame, code: str) -> pd.DataFrame:
        ...
        if not self.auto_confirm:
            preview = format_code_preview(code)
            confirmed = self._request_user_confirmation(preview, code)
            if not confirmed:
    ...
    def _request_user_confirmation(self, preview: str, full_code: str) -> bool:
        """请求用户确认是否执行代码。

        在 CLI 环境下使用 input() 交互确认；
        在非交互环境下（如被其他模块调用）默认拒绝执行。
        子类可重写此方法以适配不同的确认机制（如 GUI 弹窗）。
        """
        import sys
        ...
        print("\n" + "=" * 60)
        print("即将执行 LLM 生成的数据转换代码：")
        print("-" * 60)
        print(preview)
        print("-" * 60)
        print("此代码已通过安全校验（黑名单 + AST 白名单）。")
        try:
