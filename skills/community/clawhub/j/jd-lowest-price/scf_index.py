# -*- coding: utf-8 -*-
"""
京东自营历史最低价代理 v1.0 - 腾讯云SCF Web函数
函数名：jd-lowest-price-proxy
基于超级补贴v5.0架构，适配历史最低价频道

缓存逻辑：
- 首次请求：全量拉取京东API(~1180条，约30秒)，硬过滤后缓存
...
- 好评率筛选（默认≥97%，可自定义98/99/100）
- 销量筛选（默认≥500，MCP端控制降级）
- 价格筛选（max_price）
- 排序（score/price/drop）
- 分页（page/page_size）

降级路径（默认min_good_comments=97）：
  (97, 500) → (97, 100) → (97, 0)
  用户指定更高好评率(98/99/100)时，好评率不降级，只降销量
...
# ===== 抄底热度标签集合 =====
LOW_PRICE_TAGS = {"历史最低", "抄底价", "破价", "降价", "低价", "限时低价", "百亿补贴", "超低价", "好价"}
...
def _get_item_drop(g):
    """获取降价幅度（原价 - 到手价）"""
    ref = _get_item_ref_price(g)
    coupon = _get_item_price(g)
    return round(ref - coupon, 2) if ref > 0 else 0


def _calc_score_scf(g):
    """综合评分（满分100）
    降价幅度35 + 推广热度20 + 好评20 + 销量10 + 自营10 + 抄底热度5

    历史最低价评分特点：
    - 降价幅度是核心卖点，权重最高35
    - 好评权重提高到20（抄底也要品质保障）
    - 销量权重降低到10（最低价商品销量波动大）
    """
    drop = _get_item_drop(g)
    ...
    # 降价幅度（满分35）
    drop_score = min(drop * 2, 35)

    # 推广热度（满分20）
    comm_score = min(comm_share, 20)
    ...
    # 好评率（满分20）
    comments_score = min(good_comments / 100 * 20, 20)

    # 自营加分（满分10）
    jd_score = 10 if is_jd else 0

    # 抄底热度标签（满分5）
    tag_score = min(sum(1 for t in tags if t in LOW_PRICE_TAGS), 5)

    return round(drop_score + comm_score + sales_score + comments_score + jd_score + tag_score, 1)


def _sort_scf(items, sort_by):
    """排序：score=综合(默认)，price=到手价升序，drop=降价幅度降序"""
    if sort_by == "price":
        return sorted(items, key=lambda g: _get_item_price(g))
    elif sort_by == "drop":
        return sorted(items, key=lambda g: _get_item_drop(g), reverse=True)
    else:
...
def _filter_and_degrade(cached_data, keyword, min_gc_user):
    """好评+销量筛选，关键词结果太少时自动降级

    降级路径（min_good_comments=97默认）：
      默认:  好评≥97% + 销量≥500
      降级1: 好评≥97% + 销量≥100
      底线:  好评≥97% + 不限销量

    用户指定min_good_comments=98/99/100时，好评率不降级，只降销量。
    ...
    # 历史最低价：好评97%为默认（也是SCF硬过滤底线），只降销量
    degrade_levels = [
        (min_gc_user, 500),
        (min_gc_user, 100),
        (min_gc_user, 0),
    ...
    for gc_min, sales_min in degrade_levels:
        filtered = [g for g in kw_data
                    if float(g.get("goodCommentsShare", 0) or 0) >= gc_min
                    and int(g.get("inOrderCount30Days", 0) or 0) >= sales_min]
        if len(filtered) >= DEGRADE_THRESHOLD or (gc_min, sales_min) == degrade_levels[-1]:
            return filtered, gc_min, sales_min

    return [], min_gc_user, 0
