...
def _print_payment_card(payment_url, order_no, amount, package_name, uses, account):
    print("=" * 80)
    print("# 🦞 小龙虾主厨 · 套餐充值")
    print("=" * 80)
    print()
    ...
    print("## 📱 支付宝扫码支付")
    print()
    print(f'<iframe src="{payment_url}" width="300" height="300" frameborder="0"')
    print('  style="border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.15); border: 2px solid #1677ff; display: block; margin: 0 auto;"')
    print('  scrolling="no">')
    print("</iframe>")
    print()
    print("💡 备用方式：如 iframe 显示不完整，点击下面链接直接打开支付页面")
    print(f"[👉 点击打开支付宝付款页面]({payment_url})")
    print()
    print("-" * 80)
    print("✅ 支付页已生成。")
    print("💬 请您完成扫码支付。")
...
def create_order_and_show_payment_with_status(
    ...
    _print_payment_card(payment_url, order_no, amount, package_name, uses, account=display_account)
    return {
        "order_no": order_no,
