...
exports.checkRuleA3 = checkRuleA3;
 ...
 */
function compileLowRiskPatterns(configPatterns) {
    const regexes = [];
 ...
 */
function extractFileNamesFromTask(task) {
    const names = [];
 ...
 */
function extractPathPatternsFromTask(task) {
    const patterns = [];
 ...
 */
function isFileRelatedToTask(filePath, taskFileNames, taskPathPatterns, taskKeywords) {
    ...
    for (const taskName of taskFileNames) {
        const taskBasename = (0, path_1.basename)(taskName);
        if (taskBasename === fileName)
            return true;
    }
    ...
    for (const kw of taskKeywords) {
        if (filePathLower.includes(kw))
            return true;
    }
    return false;
}
function checkRuleA3(ctx) {
    ...
    const effectiveConfig = config ?? config_loader_1.DEFAULT_CONFIG;
    ...
    for (const file of diffFiles) {
        ...
        if (isLowRisk)
            continue;
        // 检查文件是否与任务描述相关
        const isRelated = isFileRelatedToTask(file.path, taskFileNames, taskPathPatterns, taskKeywords);
        if (!isRelated) {
    ...
    if (totalFiles > 0 && unexpectedFiles.length > totalFiles * threshold) {
        rule.status = 'WARN';
        rule.details.push(`${unexpectedFiles.length}/${totalFiles} 个文件不在任务描述 ("${task}") 范围内: ${unexpectedFiles.slice(0, 3).join(', ')}${unexpectedFiles.length > 3 ? ` 等` : ''}`);
    }
    return rule;
}
//# sourceMappingURL=rule-a3-careful-modify.js.map