---
name: "X to Kindle"
description: "Send X/Twitter posts to Kindle for distraction-free reading. Use when user shares an X/Twitter link and wants to read it on Kindle, or asks to send a tweet/thread to their Kindle device."
category: "other"
source: "ClawHub"
tags: []
platforms: []
author: ""
version: ""
license: ""
installCmd: "hermes skills install clawhub/x-kindle"
sourceUrl: "https://clawhub.ai/skills/x-kindle"
---

# X to Kindle

> Send X/Twitter posts to Kindle for distraction-free reading. Use when user shares an X/Twitter link and wants to read it on Kindle, or asks to send a tweet/thread to their Kindle device.

- **Category:** Other
- **Source:** ClawHub
- **Author:** 
- **Version:** 
- **License:** 
- **Platforms:** All
- **Install Command:** `hermes skills install clawhub/x-kindle`
- **Source URL:** [https://clawhub.ai/skills/x-kindle](https://clawhub.ai/skills/x-kindle)

## Overview


## Installation
To install this skill, run the following command in your terminal:
```bash
hermes skills install clawhub/x-kindle
```
