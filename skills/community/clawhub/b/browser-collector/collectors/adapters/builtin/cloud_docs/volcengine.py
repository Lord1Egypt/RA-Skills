#!/usr/bin/env python3
"""
collectors/adapters/builtin/cloud_docs/volcengine.py - 火山引擎文档适配器

支持: volcengine.com, www.volcengine.com, volcanoengine.com 文档结构化提取
...
class VolcengineDocAdapter(DocAdapter):
    """
    火山引擎文档适配器

    完整支持 volcengine.com 文档结构化提取。
    """
    ...
    KNOWN_DOC_SITES = {
        'volcengine.com': {
            'content_selector': '.article-content, .content-body, #article-content',
    ...
    def can_handle(self, url: str) -> bool:
        """检查是否支持此URL"""
        return any(host in url.lower() for host in self.supported_domains)
            ...
            try:
                ...
                for item in nav_items:
                    ...
                    if text and len(text) > 1:
                        ...
                        # 跳过外部链接
                        if href.startswith('http') and 'volcengine' not in href.lower() and 'volcano' not in href.lower():
                            continue
                        anchor = self._text_to_anchor(text)
                        all_headings.append((0, text, anchor))
            except Exception:
                continue
...
# 注册适配器到全局注册表
_adapter_instance = VolcengineDocAdapter()
register_adapter('volcengine_doc', _adapter_instance)
register_adapter('volcengine', _adapter_instance)
register_adapter('volcengine.com', _adapter_instance)
register_adapter('VolcanoEngine', _adapter_instance)


__all__ = ['VolcengineDocAdapter']