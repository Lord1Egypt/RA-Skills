...
| 必须展示支付卡片 | 必须展示「付款二维码图片」，禁止出现支付链接, 统一使用公共二维码生成服务直接生成 300x300px 的二维码图片显示在界面上；**自动检测平台并智能选择二维码渲染方式**（ArkClaw 使用 HTML `<img>` 标签，OpenClaw 使用 `MEDIA:` 指令）；用户可见会话中禁止出现底层嵌入实现等技术用语 |
...
| 6 | 备用方式说明更加友好，提示打开支付宝付款网页版(禁止显示支付url在界面中) | ✅ 应当 |
