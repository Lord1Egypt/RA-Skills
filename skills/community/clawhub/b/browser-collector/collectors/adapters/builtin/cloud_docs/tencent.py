#!/usr/bin/env python3
"""
collectors/adapters/builtin/cloud_docs/tencent.py - 腾讯云文档适配器

支持: cloud.tencent.com, www.cloud.tencent.com 文档结构化提取
...
class TencentDocAdapter(DocAdapter):
    """
    腾讯云文档适配器

    完整支持 cloud.tencent.com 文档结构化提取。
    """
            ...
            try:
                ...
                for item in nav_items:
                    ...
                    if text and len(text) > 1:
                        ...
                        # 跳过外部链接
                        if href.startswith('http') and 'cloud.tencent' not in href:
                            continue
...
# 注册适配器到全局注册表
_adapter_instance = TencentDocAdapter()
register_adapter('tencent_doc', _adapter_instance)
register_adapter('tencent', _adapter_instance)
register_adapter('cloud.tencent.com', _adapter_instance)


__all__ = ['TencentDocAdapter']