# -*- coding: utf-8 -*-
"""
京东自营新品首发代理 v1.0 - 腾讯云SCF Web函数
函数名：jd-new-arrivals-proxy
基于超级补贴v5.0架构，适配新品首发频道

缓存逻辑：
- 首次请求：全量拉取京东API，硬过滤后缓存
...
# ===== 新品热度标签集合 =====
NEW_ARRIVAL_TAGS = {"新品首发", "新品上市", "新品", "首发", "上新", "热销新品", "爆款", "人气新品"}
...
def _calc_score_scf(g):
    """综合评分（满分100）
    优惠力度30 + 推广热度20 + 好评20 + 销量10 + 自营10 + 热度10

    新品首发评分特点：
    - 优惠力度取代补贴力度（新品可能有首发优惠但无补贴）
    - 好评权重提高到20（新品品质更重要）
    - 销量权重降低到10（新品销量天然偏低）
    """
    ...
    is_hot = g.get("isHot", 0)
    tags = g.get("skuTags", [])

    # 优惠力度（满分30）
    discount_score = min(discount * 2, 30)

    # 推广热度（满分20）
    comm_score = min(comm_share, 20)

    # 销量（满分10）- 新品销量偏低，阈值更宽松
    if orders >= 10000:
        sales_score = 10
    elif orders >= 5000:
        sales_score = 8
    ...
    # 热度标签（满分10）- 新品标签 + isHot标记
    tag_score = min(sum(2 for t in tags if t in NEW_ARRIVAL_TAGS), 8)
    if is_hot:
...
def _sort_scf(items, sort_by):
    """排序：score=综合(默认)，price=到手价升序，discount=优惠金额降序"""
    if sort_by == "price":
