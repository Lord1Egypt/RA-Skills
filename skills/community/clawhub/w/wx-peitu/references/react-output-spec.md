...
## Background Image Compatibility (Puppeteer 截图兼容)

When generating HTML that will be screenshotted via Puppeteer, photo backgrounds MUST use `<img>` tags, NOT CSS `background-image`.

**Why**: Puppeteer headless mode may not load external images referenced via CSS `background-image: url(...)`. The `<img>` tag approach is reliable.

**Correct pattern** (for cover/back-cover with photo backgrounds):
```html
...
**Wrong pattern** (may produce blank background in screenshot):
```html
...
**Additional rules**:
- Always add `crossorigin="anonymous"` to external image `<img>` tags
- Set explicit `width` and `height` on the container div (not auto)
- Use `object-fit:cover` + `object-position` for subject alignment
- Gradient overlays go on a separate `<div>` layer between image and text
